...
(function optimizeExamFlashcardsForMemory() {
    ...
    const wxHighFreq = new Set([0, 1, 2, 3, 4, 5, 6, 7, 8, 13, 15, 16, 17, 19, 20, 21, 22]);
    applyIndexedTips(asCards(appData.wx_short_answers), [
        ...
        ["组件四件套，json开component，using引入，标签使用", ["新建组件", "component:true", "properties传参", "methods写方法", "usingComponents注册"], "高频考点：组件创建使用常考"],
        ["useExtendedLib写app，WeUI免包体积", ["app.json声明useExtendedLib", "页面usingComponents注册mp组件", "扩展库不计入包体积"], "常规考点：扩展库方式"],
        ["open-type六动作，导重切启返回退出", ["navigate/redirect", "switchTab/reLaunch", "navigateBack", "exit"], "高频考点：open-type合法值易考"],
        ["一套代码多端发，Vue生态效率高", ["可编译到App、Web、小程序", "基于Vue", "插件生态多", "节省多端开发成本"], "高频考点：uni-app优点常考"],
        ["WXML骨架，WXSS样式，JS逻辑，JSON配置", ["结构", "样式", "逻辑", "页面配置"], "高频考点：四文件组成必背"],
        ["页面建在pages，路径写进pages.json，首项是首页", ["新建.vue页面", "pages.json追加path", "path相对项目根目录", "pages第一项默认首页"], "高频考点：uni-app路径配置常考"]
    ]);
